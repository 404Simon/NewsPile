    ...
    @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($searchProfiles as $profile)
                <div
                    class="bg-white dark:bg-zinc-800 rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg border border-zinc-100 dark:border-zinc-700">
                    <div class="p-6">
                        <div class="flex justify-between items-start">
                            <a href="{{ route('profiles.show', $profile, absolute: false) }}" wire:navigate>
                                <h2
                                    class="text-xl font-bold text-zinc-900 dark:text-white truncate hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors duration-150">
                                    {{ $profile->name }}
                                </h2>
                            </a>
                            <div class="flex">
                                <flux:button href="{{ route('profiles.edit', $profile, absolute: false) }}"
                        ...
                        <div class="mt-6 space-y-4">
                            ...
                            <div>
                                ...
                                <div class="mt-2 flex flex-wrap gap-2">
                                    ...
                                    @empty
                                        <span class="text-sm text-zinc-500 dark:text-zinc-400">No news outlets
                                            selected</span>
                                    @endforelse
                                </div>
                            </div>
